...
const Cart = () => {
  ...
  const handleIncrement = (id, amount) => {
    dispatch(updateQuantity({ id, amount: amount + 1 }));
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
          <div className="md:col-span-2">
            <div className="p-4 bg-white rounded-lg shadow-md">
                  ...
                  <tbody>
                    {cartItems.map((item) => (
                      <tr className="border-b" key={item.id}>
                        ...
                        <td className="p-2 md:p-4">${(item.price * item.amount).toFixed(2)}</td>
                        <td className="p-2 md:p-4 text-right">
                          <button
              ...
              <div className="md:hidden space-y-4">
                {cartItems.map((item) => (
                  <div className="flex flex-col p-4 bg-gray-100 rounded-lg shadow-md" key={item.id}>
                    <div className="flex items-center">
                      <img src={item.img} alt={item.name} className="h-16 w-16" />
                    ...
                    <div className="mt-2 flex justify-between items-center">
                      <span>Subtotal: ${(item.price * item.amount).toFixed(2)}</span>
                      <button
                        className="rounded-full w-7 h-7 border border-gray-400 text-gray-600"
                        onClick={() => dispatch(removeFromCart(item.id))}
                      >
